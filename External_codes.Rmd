...
This files prepares the dictionary linking e-codes to the main groups used for analysis.
...
Dictionary <- Dictionary %>%
  mutate(Undetermined = ifelse ((ICD_code >= 'Y20' & ICD_code <= 'Y34'), TRUE, NA))
...
#
#Manual fix
# 33983 CVEATIY Two codes, S0602 and S0601 brief and unspecified, Dx1 and Dx5, Choose brief
# 46823 COE5UI3 Two codes, S0601 and S0602 unspecified and brief, Dx2 and Dx15, Choose unspecified
# 71932 8O#3VF1 Two codes, S0601 and S0602, brief and unspecified, Dx5 and Dx6, Choose brief
#

TBI_0 %>%
  filter(ID == 33983) %>% paste()
TBI_0 %>%
  filter(ID == 46823) %>% paste()
TBI_0 %>%
  filter(ID == 71932) %>% paste()

TBI_2$LOC_duration[TBI_2$ID == 33983 & TBI_2$LOC_duration == 'n.s.'] = NA_character_
TBI_2$LOC_duration[TBI_2$ID == 46823 & TBI_2$LOC_duration == '< 0.5'] = NA_character_
TBI_2$LOC_duration[TBI_2$ID == 71932 & TBI_2$LOC_duration == 'n.s.'] = NA_character_
